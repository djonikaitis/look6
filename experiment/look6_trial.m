...
if tid>1 && sum(strcmp(expsetup.stim.esetup_exp_version, 'task switch luminance change'))>0
    ...
    if sum(ind) == expsetup.stim.number_of_trials_per_block
        ...
        % Save temporary values, in order to over-write them
        temporary_esetup_st2_color_level = expsetup.stim.esetup_st2_color_level(tid-1);
        temporary_esetup_exp_version = expsetup.stim.esetup_exp_version{tid-1};
        
        % Over-write values temporarily
...
%% Luminance update only:
% Correction if previous trial was over-written

if exist('temporary_esetup_exp_version', 'var') && exist('temporary_esetup_st2_color_level', 'var')
    expsetup.stim.esetup_st2_color_level(tid-1) = temporary_esetup_st2_color_level; % Over-write previous trial
    expsetup.stim.esetup_exp_version{tid-1} = temporary_esetup_exp_version; % Over-write previous trial
    clear temporary_esetup_exp_version
    clear temporary_esetup_st2_color_level
end
...
% Which task it is?
ind_task = sprintf('%s', expsetup.stim.esetup_block_cond{tid});
...
while loop_over==0
    ...
    t2 = expsetup.stim.fixation_color_change_soa;
    if ~isnan(t0) && (time_current - t0 >= t1 + t2) && nansum(expsetup.stim.eframes_fixation_color_change{tid}(:, 1))~=1
        expsetup.stim.eframes_fixation_color_change{tid}(c1_frame_index1, 1) = 1;
    end
    ...
    if expsetup.stim.eframes_fixation{tid}(c1_frame_index1,1)==1
        % Properties
        if nansum(expsetup.stim.eframes_fixation_color_change{tid}(:, 1))==0
            c1 = expsetup.stim.esetup_fixation_color(tid,1:3);
        elseif nansum(expsetup.stim.eframes_fixation_color_change{tid}(:, 1))==1
            c1 = expsetup.stim.fixation_color_memory_delay;
        end
    ...
    if c1_frame_index1>1
        t_previous = expsetup.stim.eframes_texture_on{tid}(c1_frame_index1-1,1);
    else
        t_previous = expsetup.stim.eframes_texture_on{tid}(c1_frame_index1,1);
    end
    if isnan(t_previous)
        t_previous=0;
    end
    ...
    if texture_loop_counter - t_previous == 1
        if expsetup.general.recordeyes==1
    ...
    if expsetup.stim.eframes_fixation_color_change{tid}(c1_frame_index1, 1)==1 && isnan(expsetup.stim.edata_fixation_color_change(tid,1))
        if expsetup.general.recordeyes==1
            Eyelink('Message', 'fixation_color_change');
        end
        expsetup.stim.edata_fixation_color_change(tid,1) = time_current;
    end
    ...
    % Record memory offset
    if expsetup.stim.eframes_memory_off{tid}(c1_frame_index1,1)==1 && isnan(expsetup.stim.edata_memory_off(tid,1))
        if expsetup.general.recordeyes==1
            Eyelink('Message', 'memory_off');
        end
        expsetup.stim.edata_memory_off(tid,1) = time_current;
    end
