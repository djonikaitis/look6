...
stim.exp_version_temp = 'luminance change';
stim.training_stage_matrix_numbers = 1:numel(stim.training_stage_matrix);

% Stage 'fixation'. Learn to fixate while memory target is flashed.
% Stage 'look'. Learn to do look task.
% Stage 'avoid'. Learn to do avoid task.
% Stage 'luminance'. Tasks interleaved, using stimulus luminance
% Stage 'delay'. Increase the delay.
% Stage 'final'. No changes to the code. Look/avoid tasks interleaved.
