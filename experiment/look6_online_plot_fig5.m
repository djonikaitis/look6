...
if tid == 1
    ...
    if a==tid-1
        t1(1:tid-1, 1:b, 1:c, 1:d) = spikes_matrix_2(1:tid-1, 1:b, 1:c, 1:d);
    elseif a==tid
        t1(1:tid-1, 1:b, 1:c, 1:d) = spikes_matrix_2(1:tid-1, 1:b, 1:c, 1:d);
    else
...
if cond1>0
    for i = 1:size(expsetup.stim.edata_background_texture_onset_time, 2)
        if ~isnan(expsetup.stim.edata_background_texture_onset_time(tid,i))
            ...
            text(b, h_min+((h_max-h_min)*0.1), 'Tex', 'Color', [0.2, 0.2, 0.2],  'FontSize', fontszlabel, 'HorizontalAlignment', 'left')
        end
    end
end
