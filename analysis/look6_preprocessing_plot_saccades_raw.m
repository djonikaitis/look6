...
for i_subj=1:length(settings.subjects)
    ...
    for i_date = 1:numel(dates_used)
        ...
        for tid_0=1:length(ind_trials)
            ...
            % Shape
            if strcmp(S.esetup_fixation_shape{tid}, 'circle')
                v1 = 1;
            elseif strcmp(S.esetup_fixation_shape{tid}, 'square')
                v1 = 0;
            end
            ...
            if ~isnan(S.esetup_st1_coord(tid,1))
                ...
                % Shape
                if strcmp(S.esetup_target_shape{tid}, 'circle')
                    v1 = 1;
                elseif strcmp(S.esetup_target_shape{tid}, 'square')
                    v1 = 0;
                end
...
%                 end
                ...
                if  ~isnan(S.fixation_drift_maintained(tid))
%                     if S.esetup_fixation_drift_correction_on(tid,1) == 1
...
%                     end
                ...
                %============
                % Plot saccade target 1 time window
                if ~isnan(S.st2_on(tid)) && ~isnan(S.st2_off(tid))
                ...
                if ~isnan(S.memory_on(tid))
                    ...
                    if ~isnan(S.memory_off(tid))
                        ...
                        t_dur = S.esetup_memory_duration(tid);
                    end
                    h=rectangle('Position', [t_start, yc-s_window/2, t_dur, s_window],...
                ...
                if ~isnan(S.reward_on(tid))
                    ...
                    text(t_start, -8, 'Reward', 'Color', [0.7, 0.7, 0.1],  'FontSize', settings.fontszlabel, 'HorizontalAlignment', 'left')
                end
