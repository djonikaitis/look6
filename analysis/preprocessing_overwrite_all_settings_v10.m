...
function  preprocessing_overwrite_all_settings_v10(settings)
...
for i_subj = 1:length(settings.subjects)
    ...
    try
        settings = get_settings_path_and_dates_ini_v11(settings, 'path_data_combined_subject');
    ...
    for i_date = 1:length(dates_used)
        ...
        if settings.overwrite_temp_switch == 1;
            for i_folder = 1:numel(ind_folders)
                ...
                path1_copy = [settings.path_data_combined_subject, folder_name, '/' folder_name, '_original.mat'];
                ...
                elseif exist(path1, 'file') && settings.overwrite==0
                    fprintf('\nCopy of the settings file %s_original already exists, skipping analysis\n', folder_name)
                else
