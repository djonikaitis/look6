            ...
            for i_unit = 1:length(spikes_init.index_path)
                ...
                for fig1 = 1:7 % Plot figures
                    ...
                    elseif fig1==2 || fig1==3 || fig1==4 || fig1==5 || fig1==6 || fig1==7
                        ...
                        i1=size(m1,1)*3;
                        for i=1:size(m1,1)
                            index = S.em_target_coord1==m1(i,1) & S.em_target_coord2==m1(i,2) & S.em_blockcond==1 & S.em_background_texture_on==0  & S.trial_accepted==-1;
                            S.expcond(index)=i+i1;
                        end
                        % Avoid task, no texture
                        i1=size(m1,1)*4;
                        for i=1:size(m1,1)
                            index = S.em_target_coord1==m1(i,1) & S.em_target_coord2==m1(i,2) & S.em_blockcond==2 & S.em_background_texture_on==0  & S.trial_accepted==-1;
                            S.expcond(index)=i+i1;
                        end
                        % Control task, no texture
                        i1=size(m1,1)*5;
                        for i=1:size(m1,1)
                            index = S.em_target_coord1==m1(i,1) & S.em_target_coord2==m1(i,2) & S.em_blockcond==5 & S.em_background_texture_on==0  & S.trial_accepted==-1;
                            S.expcond(index)=i+i1;
                        end
...
%                         %=
                    ...
                    if ~isnan (nanmean(nanmean(nanmean(mat1)))) && (nanmean(nanmean(nanmean(mat1))))~=0
                        ...
                        elseif h_max-h_min >50
                            set(gca,'YTick', [0:20:h_max-h_min])
                        end
