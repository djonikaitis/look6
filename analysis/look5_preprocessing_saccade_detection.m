...
for i=1:length(settings.subjects)
    ...
    p1 = settings.path_data_combined;
    ...
    for i_date = 1:length(date_index)
        ...
        path3 = [settings.path_data_combined, folder_name, '/', folder_name, '_saccades.mat']; % File with raw data
        ...
        for i=1:length(t0)
            c=length(find(trial_accepted == t0(i) ));
            targettext='%s: %d trials (%d percent) \n';
            fprintf(targettext, t1{i}, c, round((c/a)*100));
            fprintf(fout, targettext, t1{i}, c, round((c/a)*100));
        end
